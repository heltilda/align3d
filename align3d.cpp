...
ccInt numLoci, numSpots, numColors, numZterms, numNeighborLengths, loopZterm, propLocus, bridgingLocus, doublePrecision, n_skip_max, alpha_0;
ccInt global_alpha_base, boundary, numMinParams, numModelRegimes, numModelTails;
ccInt *ZtermFirstLocus, *fixedLoci, *fixedSpots, *colorFirstSpot, *locusColor, *alphas, *calcs, *numSkippedColors, *neighbors, *neighborIdx, *numLociPerColor;
ccFloat max_pfn_mismatch, max_spot_overnorm, exaggeration, Zpropagated, l_step, multiplier;
ccFloat *model, *l, *p_fn, *N_difference, lastC, propDl, *spotX, *spotY, *spotZ, *spotDx, *spotDy, *spotDz, *f, *w, *grad_fw, *nlog_fw, *fw;
...
ccBool *locusMask, *spotMask, *spotWasUsed, *avoidConstrainedSpot, *breakInContour, prop_hadContourBreak, setUnboundPenalty, overBoundary, propMode, byEnumeration, calcExact;
...
int call_GetNeighbors(int argc, char **argv)
{
    ...
    getArgs(argc, argv, &spotX, &spotY, &spotZ, &spotDx, &spotDy, &spotDz, &colorFirstSpot, &neighbors, &neighborIdx,
                    byValue(&l_step), byValue(&pCutoff), &model, byValue(&numModelTails));
    
    numSpots = argInfo[0].argIndices;
    numColors = argInfo[6].argIndices - 1;
    numModelRegimes = argInfo[11].argIndices / (2*numModelTails+4);
...
int call_IterateProbs(int argc, char **argv)
{
    ...
    getArgs(argc, argv, &spotX, &spotY, &spotZ, &spotDx, &spotDy, &spotDz, &spotMappingProbs, &nlog_f, &nlog_w, &grad_fw,
                    ...
                    byValue(&l_step), &model, byValue(&numModelTails), byValue(&n_skip_max), &p_fn,
    ...
    numModelRegimes = argInfo[31].argIndices / (2*numModelTails+4);
    logsize = argInfo[41].argIndices;
    ...
    if ((f == NULL) || (w == NULL) || (Z == NULL) || (alphas == NULL) || (locusMask == NULL) || (spotMask == NULL)
            || (spotWasUsed == NULL) || (N_difference == NULL) || (numSkippedColors == NULL) || (numLociPerColor == NULL))  return 1;
    ...
    if (byEnumeration)  {  doublePrecision = 0;  setUnboundPenalty = true;  }        // high precision, grad f not currently supported in enumeration mode
    
    for (c1 = 0; c1 < numSpots; c1++)  {
        f[c1] = exp(-nlog_f[c1]);
        if (doublePrecision != 0)  checkFWbounds(f, c1);    }
    for (c1 = 0; c1 < numColors; c1++)  {
        numLociPerColor[c1] = 0;
        w[c1] = exp(-nlog_w[c1]);
        if (doublePrecision != 0)  checkFWbounds(w, c1);    }
    ...
    if (doublePrecision != 0)  {
        ...
        for (c1 = 0; c1 < 4; c1++)  {
            *mp_arrays[c1] = (mpf_t **) malloc(numLoci*sizeof(mpf_t *));
            ...
            for (i = 0; i < numLoci; i++)  {
                ccInt alpha, oneColor = locusColor[i];
                ccInt alpha_base = colorFirstSpot[oneColor];
                
                (*mp_arrays[c1])[i] = (mpf_t *) malloc((colorFirstSpot[oneColor+1] - alpha_base)*sizeof(mpf_t));
                if ((*mp_arrays[c1])[i] == NULL)  return 1;
                for (alpha = alpha_base; alpha < colorFirstSpot[oneColor+1]; alpha++)    {
...
void load_fw(const gsl_vector *current_fw)
{
    ...
    for (c1 = 0; c1 < numMinParams; c1++)  {
        ...
        if (doublePrecision != 0)  checkFWbounds(fw, c1);
    }
}
...
void checkFWbounds(double *fw, ccInt idx)
{
    if (fw[idx] < pow(DBL_MIN, 1./(n_skip_max+1)))  {  fw[idx] = pow(DBL_MIN, 1./(n_skip_max+1));  printf(" ** underflow in f[%i]\n", idx+1);  }
    else if (!isfinite(f[idx]))  {  f[idx] = DBL_MAX;  printf(" ** overflow in f[%i]\n", idx+1);  }
}
...
{
    ccInt i, alpha, alpha_base, oneColor, loopColor;
    ccFloat *first_p, C;
    ...
    if (byEnumeration)  {
        ...
        for (i = 0; i < numLoci; i++)  {
            oneColor = locusColor[i];
            alpha_base = colorFirstSpot[oneColor];
        ...
        *Z = *dC_dZ = 0.;
        ...
        if (setMasks())  {
            ...
            GetAllChains(-1, 0, ZtermWeights[loopZterm], calcExact, ccTrue, ccFalse);
        }}
    ...
    if (setMasks())  {
        ...
        if (byEnumeration)  {
            for (i = 1; i < numLoci; i++)  *dC_dZ += dC_dZ[i];          // do_dCdZ computes partial derivatives -- we use the total deriv. in Z
            for (alpha = 0; alpha < numSpots; alpha++)  spotWasUsed[alpha] = ccFalse;
            GetAllChains(-1, 0, ZtermWeights[loopZterm], calcExact, ccTrue, ccTrue);    }
...
{
    ccInt i, n_skip, oneColor, loopColor, nSkip, boundary_overshoot, alpha, other_boundary, alpha_base, alpha_top, BICoffset;
    ...
    if (hasSpots(i))  {
        ...
        if ((i < 0) || (i >= numLoci))  {
            ...
            alpha_base = colorFirstSpot[oneColor];
            alpha_top = colorFirstSpot[oneColor+1];            }
...
{
    ccInt i, alpha, alpha_base, oneColor;
    ...
    if (hasSpots(i))  {
        ...
        for (alpha = alpha_base; alpha < colorFirstSpot[oneColor+1]; alpha++)  {
            ...
            dC_dp_p_fn_rate *= N_difference[oneColor] / numLociPerColor[oneColor] / max_pfn_mismatch / max_pfn_mismatch;
...
void GetAllChains(ccInt prevLocus, ccInt i, ccFloat pMapping, ccBool noOverlapsAtAll, ccBool hadContourBreak, ccBool gradientMode)
{
    if (i == numLoci)  {
        
        ccInt j, loopColor;
        
        if (!gradientMode)  *Z += pMapping;
...
//if (j_alpha >= j_alpha_base)  printf(" %i", j_alpha+1);  else  printf(" 0");
            if (j_alpha >= j_alpha_base)  {
                if (!gradientMode)  LL_Double(p + j, 1)[j_alpha - j_alpha_base] += pMapping;
                else  {
                    ccFloat dC_dp = N_difference[j_color] / numLociPerColor[j_color] / max_pfn_mismatch / max_pfn_mismatch;
                    for (loopColor = 0; loopColor < numColors; loopColor++)  {
                        grad_fw[loopColor] += (pMapping*numSkippedColors[loopColor]/w[loopColor]) * dC_dp / (*Z);
        }   }   }   }
        
        for (loopColor = 0; loopColor < numColors; loopColor++)  {
            grad_fw[loopColor] += (pMapping*numSkippedColors[loopColor]/w[loopColor]) * (*dC_dZ);   }
//printf(" ) -> %g\n", pMapping);
    }
    
    else  {
        
        ccInt oneColor = locusColor[i];
        ccInt alpha_base = colorFirstSpot[oneColor], alpha;
        
        alphas[i] = alpha_base-1;
        if (i - prevLocus <= n_skip_max)  {
        if (mappingIsAllowed(i, -1, -1, ccFalse))  {
            numSkippedColors[oneColor]++;
            GetAllChains(prevLocus, i+1, pMapping * w[oneColor], noOverlapsAtAll, hadContourBreak || breakInContour[i], gradientMode);
            numSkippedColors[oneColor]--;
        }}
        
        for (alpha = alpha_base; alpha < (ccInt) colorFirstSpot[oneColor+1]; alpha++)  {
        if (mappingIsAllowed(i, alpha, prevLocus, noOverlapsAtAll))  {
            ...
            GetAllChains(i, i+1, pMapping * extra_p, noOverlapsAtAll, breakInContour[i], gradientMode);
...
int call_GaussianChain(int argc, char **argv)
{
    ...
    getArgs(argc, argv, byValue(&mode), &spotX, &spotY, &spotZ, &spotDx, &spotDy, &spotDz,
                byValue(&L), &model, byValue(&numModelTails), byValue(&FD1), byValue(&FD2), &result);
    
    numSpots = argInfo[1].argIndices;
    ...
    exaggeration = 1.;
    calcs = &dummy_calcs;
    numModelRegimes = argInfo[8].argIndices / (2*numModelTails+4);
...
{
    ccInt loopModelRegime, loopModelTail;
    ccFloat invDecayConstant2, ax, ay, az, ans, expectedDistance, *modelCopy = model, spotDistSq, GaussPrefactor;
    const ccFloat pi3 = pi*pi*pi;
    
    (*calcs)++;
    
    for (loopModelRegime = 1; loopModelRegime < numModelRegimes; loopModelRegime++)  {
        if (dl < modelCopy[(2*numModelTails+4)])  break;
        modelCopy += (2*numModelTails+4);     }
    ...
    spotDistSq = sqSpotDistance(imageSpot1, imageSpot2, ax, ay, az);
    GaussPrefactor = sqrt(ax*ay*az/pi3);
    if (GN_mode == 0)
        ans = GaussPrefactor*exp(-spotDistSq);
    else if (GN_mode == 1)
        ans = exp(-spotDistSq);
    else
        ans = GaussPrefactor;
    ...
    if ((GN_mode <= 1) && (numModelTails > 0))  {
        ans *= modelCopy[3];
        spotDistSq = sqSpotDistance(imageSpot1, imageSpot2, 1., 1., 1.);
        for (loopModelTail = 0; loopModelTail < numModelTails; loopModelTail++)  {
            ccInt tailIdx = 2*loopModelTail + 4;
            ccFloat toAdd = modelCopy[tailIdx] * exp(-modelCopy[tailIdx+1] * sqrt(spotDistSq));
            if (GN_mode == 1)  toAdd /= GaussPrefactor;       // to keep the relative proportions the same
            ans += toAdd;
    }   }
...
{
    ccInt c1, alpha_base, alpha, oneColor, *C2F;
    arg_info *argInfo = (arg_info *) *(argv+argc);
